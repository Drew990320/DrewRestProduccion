...
export declare class CreditosController {
    private readonly creditos;
    constructor(creditos: CreditosService);
    listar(todos?: string): Promise<{
        id_credito: number;
        id_pedido: number;
    ...
    crear(dto: CrearCuentaCreditoDto, req: {
        user: {
            id: number;
        };
    }): Promise<{
    ...
    abono(id: number, dto: AbonoCuentaCreditoDto): Promise<{
        id_credito: number;
        id_pedido: number;
        id_factura: number | null;
        mesa_numero: number;
