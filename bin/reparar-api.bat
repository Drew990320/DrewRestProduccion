...
echo ========================================
echo   DrewRest - Reparar API
echo ========================================
echo.

if not exist "%ROOT%\api\package.json" (
  echo [ERROR] No existe api\package.json
  echo Copia la carpeta DrewRest\api completa desde el paquete de desarrollo.
  pause
  exit /b 1
)
...
if not exist "%ROOT%\api\vendor\shared-domain\dist\index.js" (
  ...
  echo.
  echo En desarrollo: npm run drewrest:empaquetar
  pause
  exit /b 1
)
...
if not exist "%ROOT%\api\node_modules\@drewrest\shared-domain\dist\usuario-display.js" (
  if not exist "%ROOT%\api\node_modules\@drewrest\shared-domain\package.json" (
    echo.
    echo [AVISO] Sigue faltando @drewrest/shared-domain en node_modules.
...
echo.
echo Listo. Cierra DrewRest_API si estaba abierto y ejecuta inicio.bat
echo.
pause
